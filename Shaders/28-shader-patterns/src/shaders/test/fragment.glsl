...
// no true random in GLSL so implement a wack value
...
/*     vec2 gridUv = vec2(floor(vUv.x * 10.0) / 10.0, floor((vUv.y + vUv.x) * 10.0) / 10.0);
    ...
    // pattern 27
    // approach 1: float strength = length(vUv - .5);
    // approach 2: positions shadow wherever you want using vec2 coords- float strength = distance(vUv, vec2(.5, .5));
    ...
    // pattern 29
    // point light texture
    // float strength = 0.015 / distance(vUv, vec2(.5, .5));

    // pattern 30
/*     vec2 lightUv = vec2(vUv.x * 0.1 + 0.45, vUv.y * 0.5 + 0.25);
    float strength = 0.015 / distance(lightUv, vec2(.5, .5));
     */

    // pattern 31
/*     vec2 lightUvX = vec2(vUv.x * 0.1 + 0.45, vUv.y * 0.5 + 0.25);
    float lightX = 0.015 / distance(lightUvX, vec2(.5, .5));

    vec2 lightUvY = vec2(vUv.y * 0.1 + 0.45, vUv.x * 0.5 + 0.25);
    float lightY = 0.015 / distance(lightUvY, vec2(.5, .5));
...
/*     vec2 rotatedUv = rotate(vUv, PI / 4.0, vec2(0.5, 0.5));
    ...
    // pattern 33
    // float strength = step(.25, distance(vUv, vec2(0.5)));
